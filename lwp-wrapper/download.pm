...
#         $self->{ua}->show_progress( $self->{progress} );
        ...
        if ($size == File::Stat->new($filename)->size) {
            say STDERR "file with same filename and same size already exists";  # commented
            if ($self->{log}) {
                say STDERR "\"$filename\"";
            }
        }
...
#                     print STDERR "\e[2A";
                ...
                carp(RED . "ERROR while downloading $args{url}" . RESET);   # commented
...
#                     print STDERR "\e[2A";
                ...
                carp("LOCATION file exists but, local file size != url file, overwrite option: false => trying to downloading it");
                carp(RED . "ERROR while downloading $args{url}" . RESET);   # commented
                say STDERR RED, $args{url}, RESET, "\n";
            }
        }
...
#             carp(RED . "ERROR while downloading $args{url}" . RESET);   # commented
            carp("LOCATION file does not exists, trying to downloading it");
            carp("ERROR while downloading $args{url}");   # commented
            say STDERR RED, $args{url}, RESET, "\n";
        }
    }
...
london@archlinux:~
$ perl -Mdd -Mdownload -E 'my $download=download->new; $download->file(url => shift, filename => "text.txt", log => 1)' "https://cpp.hotexamples.com/examples/-/-/g_hash_table_new_full/cpp-g_hash_table_new_full-function-examples.html"
** HEAD https://cpp.hotexamples.com/examples/-/-/g_hash_table_new_full/cpp-g_hash_table
Use of uninitialized value $size in numeric eq (==) at /home/london/.my_configurations/perl_modules/download.pm line 143.
** GET https://cpp.hotexamples.com/examples/-/-/g_hash_table_new_full/cpp-g_hash_table_
london@archlinux:~
$ qqq
perl -Mdd -Mdownload -E 'my $download=download->new; $download->file(url => shift, filename => "text.txt", log => 1)' "https://cpp.hotexamples.com/examples/-/-/g_hash_table_new_full/cpp-g_hash_table_new_full-function-examples.html"
perl -Mdd -Mdownload -E 'my $download=download->new; $download->file(url => shift, filename => "text.txt", log => 1)' "https://cpp.hotexamples.com/examples/-/-/g_hash_table_new_full/cpp-g_hash_table_new_full-function-examples.html"
